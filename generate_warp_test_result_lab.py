...
parser.add_argument('--run',       type=int, default=57,     help="Run ID. In case you want to run replicates")
...
fold_list_4_image_path = IPMI_data_pickle_save_path + 'LAB_CE+HE_fold_list_4_image.txt'
...
if Type == 'Run_cls' and args.train > 0:
    ...
    FLAG_TEST_FOLD = 4
    _, labels, _, _, _ = ut.get_dataset_from_idx_file_and_label_from_Id_attr(LAB_data_name_list_txt, LAB_path_group_data, Id_attr, normalize=False, mask_dataset=True, data_source='lab')
    ...
    f.close()  
    file_idx = np.genfromtxt(LAB_data_name_list_txt, dtype='str') 
    ...
    filename_of_data_fold = ut.get_filename_of_data_fold_from_fold_list_4_image(LAB_data_name_list_txt, fold_list_4_image, labels, num_cls=2, fold_num=5)
...
def save_data_to_nii(folder_dir, file_name, data, fold, save_type='both'):
    if save_type == 'both':
        ...
        name_prefix = ['NC', 'EHE', 'SIMU_EHE', 'SIMU_NC', 'CYC_BACK_NC', 'CYC_BACK_EHE', 'D_Field_SIMU_EHE', 'D_Field_SIMU_NC', 'D_Field_CYC_BACK_NC', 'D_Field_CYC_BACK_EHE']
        ...
        np.savetxt(folder_dir + 'EHE.txt', AD_FILE_NAME, '%s')
        ...
        for i in range(num_of_NC): 
            ...
            NC_filename = folder_path + prefix + '/' + prefix + '_' + NC_NAME
            NAME_TXT[8].append(prefix + '_' + NC_NAME)
            nib.save(nib_img, NC_filename)      
...
def simulate_img_fold(cyc_con, data_fold, label_fold, filename_of_data_fold, data_save_path, eval_fold_list=[0,1,2,3,4], requires_grad=False, mode='eval', verbose=True):
    ...
    for fold in eval_fold_list:
        ...
        data = [xn, xp, s3, s1, s4, s2, v3, v1, v4, v2]
        if DEBUG_VERBOSE == True:
            print('v1:', v1.shape)
            print('v3:', v3.shape)
        # sleep(1000)
