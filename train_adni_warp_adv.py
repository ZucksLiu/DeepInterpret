...
# from codebase.models.vae_relu import VAE3d, Siamese_Network
# from codebase.train import train, train3d, train3d_1,train3d_2, test3d_tsne, test3d_tsne_draw, test3d_tsne_z, tsne_z_mean, tsne_z_mean_new,test3d_tsne_z_Subject_Id,train3d_7
# from codebase.train import test3d_tsne_z_Subject_Id_hline, train3d_8, train3d_9, train3d_10, train3d_13
# from codebase.train import train3d_cls, train3d_cls_vanilla
# from codebase.train import test3d_tsne_z_Subject_Id_10, test3d_tsne_z_Subject_Id_10_hline,test3d_tsne_z_Subject_Id_10_scale
# from codebase.models.nns.v5 import Distance_Relu_Loss 
# from codebase.models.nns.v4 import Distance_Relu_Loss 
# from codebase.models import nns
# from codebase.models.ae_relu_weight import Classifier
...
parser.add_argument('--run',       type=int, default=62,     help="Run ID. In case you want to run replicates")
...
def train_cls(cls, dataset, label, test_data, test_label, device, tqdm, fold, iter_max, iter_save,  lr_decay_step, schedule=False):
            ...
            while True:
                ...
                for k in range(fold-1):
                    ...
                    for batch_idx, (xu) in enumerate(train_loader):
                        ...
                        # optimizer.zero_grad()
                        # loss = cls(xu_f, l, 'train')
                        # loss.backward()
                        # optimizer.step()
                        # print("Finish Backward flipped")
                        pbar.set_postfix(
                                loss='{:.2e}'.format(loss))
                        pbar.update(1)
                        ...
                        if i % iter_save == 0:
                            ...
                            # ut.save_model_by_name_cls(model.vae, i)
                            # ut.save_model_by_name_ae(model.relu_loss,i)
                            # ut.sample_image(model.vae, 4, dataset_mean, dataset_std, path_saveimg, str(i))
                if i >= iter_max:
                # if i >= 4:
                    path = "/scratch/users/zucks626/ADNI/IPMI/checkpoints/"
                    save_dir = path + model.name + "/"
    ...
    #         xu = torch.tensor(x[j*Batch_Num:(j+1)*Batch_Num])
    #         d = torch.tensor(y[j*Batch_Num:(j+1)*Batch_Num])
    #         optimizer.zero_grad()
    #         loss, acc= cls(xu, d)
    #         acc_sum += acc
    #         gl.append(loss.detach().numpy())
    #         loss.backward()
    #         optimizer.step()
    #     losses.append(sum(gl)/len(gl))
    #     acc_list.append(acc_sum/(num_of_batch * Batch_Num))
    #     print('iter:', i,' loss:',sum(gl)/len(gl), ' acc:',acc_sum/(num_of_batch * Batch_Num))
...
def main():
    ...
    # train_cls(cls, train_loader_list, train_label_loader_list, test_loader_list, test_label_loader_list, device=device, tqdm=tqdm.tqdm, fold=5, iter_max=1010, iter_save=50, lr_decay_step=1500, schedule=False)
    # sleep(1000)
    global_step = 400
    ...
    # dtrans = CD_Simu(name='dtrans', device=device)
    # ctrans = CD_Simu(name='ctrans', device=device)
    # cyc_con = Cycle_Cons(cls, ctrans, dtrans, name=cyc_con_model_name, device=device)
    # FLAG_COND_TYPE = 'Cond_sim'
    FLAG_COND_TYPE = 'Cond'
    trans = Convnet_SkipConnection(in_ch=1, out_ch=1, name='trans', device=device, z_dim=args.z, conv_type=FLAG_COND_TYPE)
    ...
    # num_of_batch = N*2 // Batch_Num
    # optimizer = optim.Adam(filter(lambda p: p.requires_grad, cyc_con.parameters()), lr=2e-4, weight_decay=1e-4)
    # optimizer_1 = optim.Adam(filter(lambda p: p.requires_grad, cyc_con.parameters()), lr=1e-3, weight_decay=1e-4)
    # optimizer_2 = optim.Adam(filter(lambda p: p.requires_grad, cyc_con.parameters()), lr=1e-3, weight_decay=1e-4)
    # optimizer_3 = optim.Adam(filter(lambda p: p.requires_grad, cyc_con.parameters()), lr=2e-4, weight_decay=1e-4)
    # optimizer_4 = optim.Adam(filter(lambda p: p.requires_grad, cyc_con.parameters()), lr=1e-3, weight_decay=1e-4)
    # optimizer_5 = optim.Adam(filter(lambda p: p.requires_grad, cyc_con.parameters()), lr=1e-3, weight_decay=1e-4)
    ...
    #     idx = idx[:64]
    #     idx1 = np.random.permutation(xn.shape[0])
    #     idx1 = idx1[:64]

    #     if i % 100 == 0:
    #         neg_lp_loss, dtrans_bce_loss, cdtrans_loss, acc_d = cyc_con.forward_d(xn)
    #         pos_lp_loss, ctrans_bce_loss, dctrans_loss, acc_c = cyc_con.forward_c(xp)
    #         simulated_1 = cyc_con.ctrans(xp)
    #         simulated_2 = cyc_con.dtrans(simulated_1)
    #         simulated_3 = cyc_con.dtrans(xn)
    #         simulated_4 = cyc_con.ctrans(simulated_3)
    #         simulated_list_1.append(simulated_1.detach().numpy())
    #         simulated_list_2.append(simulated_2.detach().numpy())
    #         simulated_list_3.append(simulated_3.detach().numpy())
    #         simulated_list_4.append(simulated_4.detach().numpy())
    #         print(i,'d_acc',acc_d/N,'c_acc',acc_c/N,'dc_loss',dctrans_loss.data.numpy(),'cd_loss',cdtrans_loss.data.numpy())
    #         print('c_lp:',neg_lp_loss.data.numpy(), 'd_lp:',pos_lp_loss.data.numpy(),'bce_c_loss:',ctrans_bce_loss.data.numpy(), 'bce_d_loss:',dtrans_bce_loss.data.numpy()) 
            
    #     for j in range(5):
    #         optimizer.zero_grad()
    #         pos_lp_loss, ctrans_bce_loss, dctrans_loss, acc_c = cyc_con.forward_c(xp[idx])
    #         ctrans_bce_loss = w_clf_c * ctrans_bce_loss
    #         ctrans_bce_loss.backward()
    #         optimizer.step()
    #     for j in range(3):
    #         optimizer_1.zero_grad()
    #         pos_lp_loss, ctrans_bce_loss, dctrans_loss, acc_c = cyc_con.forward_c(xp[idx])
    #         dctrans_loss = w_dctrans * dctrans_loss
    #         dctrans_loss.backward()
    #         optimizer_1.step()  
    #     for j in range(1):
    #         optimizer_2.zero_grad()
    ...
    #     for j in range(3):
    #         optimizer_3.zero_grad()
    #         neg_lp_loss, dtrans_bce_loss, cdtrans_loss, acc_d = cyc_con.forward_d(xn[idx1])
    #         dtrans_bce_loss = w_clf_d * dtrans_bce_loss
    #         dtrans_bce_loss.backward()
    #         optimizer_3.step()
    #     for j in range(3):
    #         optimizer_4.zero_grad()
    #         neg_lp_loss, dtrans_bce_loss, cdtrans_loss, acc_d = cyc_con.forward_d(xn[idx1])
    #         dctrans_loss = w_cdtrans * cdtrans_loss
    #         dctrans_loss.backward()
    #         optimizer_4.step()  
    #     for j in range(1):
    #         optimizer_5.zero_grad()
    #         neg_lp_loss, dtrans_bce_loss, cdtrans_loss, acc_d = cyc_con.forward_d(xn[idx1])
    #         w_cw = w_cw * neg_lp_loss
    #         neg_lp_loss.backward()
    #         optimizer_5.step()  
